...
router.get("/messages", async (req: Request, res: Response) => {
  ...
  // When a specific target is requested, fetch messages that target exactly
  // that audience OR target "all" (broadcast). When target=all, fetch everything.
  const targetCondition = target === "all"
    ? undefined
    : eq(globalMessagesTable.target, target);

  const broadcastCondition = target === "all"
    ? undefined
    : eq(globalMessagesTable.target, "all");
  ...
  // Post-filter: keep messages that match the requested target or are broadcast to all
  const filtered = target === "all"
    ? items
    : items.filter((m) => m.target === target || m.target === "all");
