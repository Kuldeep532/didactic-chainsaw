...
const activity = [
  ["09:42", "Sales Agent", "Qualified Acme Labs as a high-value lead"],
  ["09:36", "Research Agent", "Found 3 competitor pricing changes"],
  ["09:21", "Finance Agent", "Flagged a 14% month-over-month spend increase"],
  ["09:04", "Marketing Agent", "Prepared a personalized campaign draft"],
];
        ...
        <aside className="w-full border-b border-slate-800 bg-slate-950 text-white lg:min-h-screen lg:w-64 lg:border-b-0" aria-label="Nexus navigation">
          <div className="flex items-center justify-between px-5 py-5">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-lg font-bold">Nexus</p><p className="text-xs text-slate-400">AI Workforce</p></div>
            </div>
          ...
          <nav className="flex gap-1 overflow-x-auto px-3 pb-3 lg:block lg:space-y-1" aria-label="Primary">
            {["Overview", "AI Workforce", "Goals", "Approvals", "Activity", "Integrations"].map((item, index) => (
              <button key={item} className={`flex min-w-max items-center gap-3 rounded-lg px-3 py-2.5 text-sm lg:w-full ${index === 0 ? "bg-white text-slate-950" : "text-slate-300 hover:bg-slate-800"}`}>
                <LayoutDashboard size={16} aria-hidden="true" />{item}
              </button>
            ))}
          </nav>
          <div className="m-4 hidden rounded-xl border border-slate-800 bg-slate-900 p-4 lg:block">
            <div className="flex items-center gap-2 text-xs font-semibold"><ShieldCheck size={15} /> Governance</div>
        ...
        <section className="min-w-0 flex-1">
          <header className="border-b border-slate-200 bg-white px-5 py-4 lg:px-8">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Business command center</p><h1 className="mt-1 text-2xl font-bold tracking-tight">Good morning. Nexus is on it.</h1></div>
              <div className="flex gap-2"><button className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold"><Globe2 size={16} /> Live systems</button><button className="inline-flex items-center gap-2 rounded-lg bg-slate-950 px-3 py-2 text-sm font-semibold text-white"><Plus size={16} /> Add agent</button></div>
            </div>
          </header>
          ...
          <div className="mx-auto max-w-7xl space-y-6 p-5 lg:p-8">
            <section className="rounded-2xl bg-slate-950 p-6 text-white lg:p-8" aria-labelledby="goal-heading">
              <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div className="max-w-2xl"><div className="mb-3 flex items-center gap-2 text-sm font-semibold text-slate-300"><Zap size={16} /> Autopilot {autopilot ? "active" : "paused"}</div><h2 id="goal-heading" className="text-3xl font-bold tracking-tight">Tell Nexus what outcome you want.</h2><p className="mt-3 text-sm leading-6 text-slate-400">Nexus turns a business goal into a plan, assigns specialist agents, executes approved actions, verifies results, and optimizes.</p></div>
                <button onClick={() => setAutopilot((value) => !value)} aria-pressed={autopilot} className="inline-flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm font-semibold"><Activity size={16} /> {autopilot ? "Pause autopilot" : "Resume autopilot"}</button>
              </div>
              <div className="mt-7 flex flex-col gap-3 rounded-xl border border-slate-800 bg-slate-900 p-3 sm:flex-row">
                <label htmlFor="nexus-goal" className="sr-only">Business goal</label><input id="nexus-goal" value={goal} onChange={(event) => setGoal(event.target.value)} onKeyDown={(event) => event.key === "Enter" && runGoal()} placeholder="e.g. Increase qualified sales by 30% this quarter" className="min-w-0 flex-1 bg-transparent px-3 py-3 text-sm text-white outline-none placeholder:text-slate-500" />
                <button onClick={runGoal} disabled={running || !goal.trim()} className="inline-flex items-center justify-center gap-2 rounded-lg bg-white px-5 py-3 text-sm font-bold text-slate-950 disabled:opacity-50"><Play size={16} fill="currentColor" /> {running ? "Planning…" : "Run goal"}</button>
              </div>
            </section>

            <section aria-labelledby="metrics-heading"><h2 id="metrics-heading" className="sr-only">Business metrics</h2><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {[["Active agents", "6", "+2 this week", Bot], ["Tasks completed", "184", "+21%", Check], ["Hours saved", "37.5", "this month", Clock3], ["AI spend", "₹8,420", "18% under budget", WalletCards]].map(([label, value, detail, Icon]) => <article key={String(label)} className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex justify-between"><p className="text-sm text-slate-500">{label}</p><Icon size={17} className="text-slate-400" aria-hidden="true" /></div><p className="mt-3 text-2xl font-bold">{value}</p><p className="mt-1 text-xs font-semibold text-slate-500">{detail}</p></article>)}
            </div></section>

            <div className="grid gap-6 xl:grid-cols-[1.45fr_1fr]">
              <section className="rounded-xl border border-slate-200 bg-white" aria-labelledby="workforce-heading"><div className="border-b border-slate-200 px-5 py-4"><h2 id="workforce-heading" className="font-bold">Your AI workforce</h2><p className="mt-1 text-xs text-slate-500">Specialists coordinating toward your goals</p></div><div className="grid gap-px bg-slate-200 sm:grid-cols-2">
                {agents.map((agent) => { const Icon = agent.icon; return <button key={agent.id} onClick={() => setSelectedAgent(agent.id)} aria-pressed={selectedAgent === agent.id} className={`bg-white p-5 text-left hover:bg-slate-50 ${selectedAgent === agent.id ? "ring-2 ring-inset ring-slate-950" : ""}`}><div className="flex items-start justify-between"><span className="grid size-10 place-items-center rounded-lg bg-slate-100"><Icon size={19} /></span><span className={`rounded-full px-2 py-1 text-[11px] font-bold ${agent.status === "Working" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{agent.status}</span></div><p className="mt-4 font-bold">{agent.name}</p><p className="mt-1 text-xs text-slate-500">{agent.role}</p><p className="mt-3 text-xs font-semibold">{agent.metric}</p></button>; })}
              </div></section>
